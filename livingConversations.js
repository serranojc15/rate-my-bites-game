const livingDinnerStory = {
  ...
  events: [
    ...
    },
    {
      ...
      kind: "pup",
      speaker: "Pup",
      text: "The room has shifted. Review the evidence, set your confidence, and make your prediction.",
      emotion: "decisive",
      cameraTarget: "pup"
    }
  ]
...
function storyMemoryRecord(event) {
  ...
  if (!state.storyMemory.some(item => item.eventId === event.id)) {
    state.storyMemory.push({ eventId: event.id, ...event.memory });
  }
}
...
function livingConversation() {
  ...
  app.innerHTML = `<section class="living-stage camera-${event.cameraTarget} ${isConfessional ? "living-confessional" : ""}">
    ...
    <div class="living-footer">
      ...
      <button class="primary-button" id="nextLiving">${state.conversationIndex === livingDinnerStory.events.length - 1 ? "Begin Investigation" : "Continue"}</button>
    </div>
  </section>`;
...
initialState = function () {
  return {
    ...sprint42InitialState(),
    conversationIndex: 0,
    livingConfidence: { "group-restaurant": 3, "emma-meal": 3, "marcus-meal": 3, "olivia-drink": 3 },
    appliedInfluence: [],
    storyMemory: []
  };
};

const sprint41AdvanceBriefing = advanceBriefing;
...
document.addEventListener("keydown", event => {
  if (state.screen === "conversation" && (event.key === " " || event.key === "ArrowRight")) {
