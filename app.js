...
function bindPeopleCards() {
  app.querySelectorAll("[data-person]").forEach(el => el.onclick = event => {
    if (event.target.closest("button") && !event.target.closest("[data-person]")) return;
    openPersonCard(diners.find(person => person.id === el.dataset.person));
  });
}

function preferencePanel(person) {
  if (person.permission === "none") return `<div class="privacy-message"><strong>🔒 Preferences are private</strong><p>${person.name} has not given permission to share detailed food or drink preferences. You can still use the clues shared for this game.</p></div>`;
  const limited = person.permission === "limited";
  return `<div class="preferences-panel"><div class="privacy-banner ${limited ? "limited" : "shared"}"><strong>${limited ? "◐ Limited sharing" : "✓ Shared with this group"}</strong><span>${limited ? `${person.name} chose to share only a few preferences.` : `${person.name} gave permission to show these preferences.`}</span></div><h3>Food & drink preferences</h3><div class="preference-chips">${person.preferences.map(item => `<span>${escapeHtml(item)}</span>`).join("")}</div>${person.dislikes.length ? `<h3>Usually avoids</h3><div class="preference-chips avoid">${person.dislikes.map(item => `<span>${escapeHtml(item)}</span>`).join("")}</div>` : ""}${limited ? `<p class="privacy-footnote">Additional preferences remain private.</p>` : ""}</div>`;
}
...
function openPersonCard(person) {
  ...
  modal.innerHTML = `<div class="modal-backdrop" data-close></div><section class="person-modal-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(person.name)} profile"><button class="modal-close" data-close aria-label="Close">×</button>${photo(images.people[person.id], person.name, "modal-person-photo")}<div class="modal-person-content"><span class="role-tag">${person.role}</span><h2>${person.name}</h2><p>${person.intro}</p><div class="permission-row"><span class="permission-badge ${person.permission}">${person.permission === "full" ? "✓" : person.permission === "limited" ? "◐" : "🔒"} ${person.permissionLabel}</span></div><div class="profile-lines"><div><span>Favorite</span><strong>${person.favorite}</strong></div><div><span>Fun fact</span><strong>${person.funFact}</strong></div></div><div class="fact-list">${person.facts.map(fact => `<div class="fact">${fact}</div>`).join("")}</div><button class="secondary-button wide preference-toggle" type="button">${person.permission === "none" ? "View preference privacy" : "View shared preferences"}</button><div class="preference-slot hidden">${preferencePanel(person)}</div></div></section>`;
  ...
  modal.querySelector(".preference-toggle").onclick = () => modal.querySelector(".preference-slot").classList.toggle("hidden");
...
function intro() {
  ...
  app.innerHTML = `<div class="intro-stage"><div class="intro-top"><p class="eyebrow">Meet Tonight’s People</p><button class="text-button" id="skip">Skip intro</button></div>${hostCard(state.introIndex === 0 ? "Welcome back, Biters! Tonight, three people are planning dinner together. Pay attention—every detail could matter." : `Next up is ${person.name}. What do their habits tell you about tonight?`)}<article class="person-feature clickable-person" data-person="${person.id}">${photo(images.people[person.id], person.name, "person-hero")}<div class="person-copy"><span class="role-tag">${person.role}</span><h1>${person.name}</h1><p>${person.intro}</p><div class="tap-hint">Tap ${person.name} anytime to reopen this card</div><div class="profile-lines"><div><span>Favorite</span><strong>${person.favorite}</strong></div><div><span>Fun fact</span><strong>${person.funFact}</strong></div></div><div class="fact-list">${person.facts.map(fact => `<div class="fact">${fact}</div>`).join("")}</div></div></article><div class="intro-dots">${diners.map((_, i) => `<span class="${i === state.introIndex ? "active" : ""}"></span>`).join("")}</div><div class="actions"><button class="primary-button" id="nextIntro">${state.introIndex === diners.length - 1 ? "Predict the Group Restaurant" : "Meet the Next Person"}</button></div></div>`;
...
function personMini(person, clue) { return `<article class="person-mini clickable-person" data-person="${person.id}" tabindex="0" role="button" aria-label="Open ${person.name} card">${photo(images.people[person.id], person.name)}<div><strong>${person.name}</strong><small>${clue}</small><span class="view-card">View card →</span></div></article>`; }
...
function play() {
  ...
  app.innerHTML = `${progress()}${hostCard(person.clues[stage])}<div class="round-person clickable-person" data-person="${person.id}" tabindex="0" role="button">${photo(images.people[person.id], person.name)}<div><span class="role-tag">${person.role}</span><h2>${person.name}</h2><p>What will ${person.name} order at ${restaurant.name}?</p><span class="view-card">Review card & preferences →</span></div></div><div class="choice-grid">${availableOptions(stage).map(option => `<button class="food-option ${selected === option.value ? "selected" : ""}" data-value="${option.value}">${photo(option.image, option.value)}<span><strong>${option.value}</strong><small>${labels[stage]} at ${restaurant.name}</small></span></button>`).join("")}</div><p class="lock-note">You may change this prediction until you continue.</p><div class="actions"><button class="primary-button" id="continue" ${selected ? "" : "disabled"}>${state.dinerIndex === diners.length - 1 ? `Finish ${labels[stage]} Round` : `Lock ${person.name}`}</button></div>`;
